...
export function AddUserDialog({ open, onOpenChange, onAddUser, onUserCreated }) {
  ...
  // Handle next button click to move to password tab
  const handleNext = (e) => {
    // Prevent any default behavior
    e?.preventDefault();
    e?.stopPropagation();

    // Validate first section before moving to next tab
    const sectionErrors = {}

    if (!formData.name.trim()) sectionErrors.name = "Name is required"
    if (!formData.email.trim()) {
      sectionErrors.email = "Email is required"
    } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) {
      sectionErrors.email = "Invalid email format"
    }
    if (!formData.role) sectionErrors.role = "Role is required"
      ...
      <DialogContent className="sm:max-w-[600px]">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-4">
          ...
          <TabsContent value="password" className="space-y-4 pt-4">
            {/* Wrap only the password section in a form */}
        ...
        {/* Show Next button only when on details tab, outside of any form */}
        {activeTab === "details" && (
          <DialogFooter className="mt-6 gap-2">
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)}>
              Cancel
